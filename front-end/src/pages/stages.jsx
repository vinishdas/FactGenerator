...
function Stages() {
  ...
  const filteredStages = filter === "All" 
    ? disease.stages 
    : disease.stages.filter(s => s.status === filter);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900">{disease.name} Stages</h1>
            <p className="text-slate-500 mt-2 text-lg">Detailed progression analysis and fact verification.</p>
          </div>

          <div className="flex bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200 self-start">
            {["All", "Early", "Intermediate", "Advanced"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
              ...
              >
                ...
                <div className="relative z-10">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${
                    stage.severity === 'Severe' || stage.severity === 'Extreme' 
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    ...
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                      stage.severity === 'Low' ? 'bg-emerald-50 border-emerald-100 text-emerald-600' :
