...
// 3. CODE COVERAGE BAR
const CodeBar = ({ label, count, total, color }) => {
  const percentage = Math.round(((count || 0) / total) * 100);
...
const SRRelevanceWidget = ({ stats, onClick }) => {
  ...
  const sr2Percent = total === 0 ? 0 : (s2 / total) * 100;
  const sr1Percent = total === 0 ? 0 : (s1 / total) * 100;
  const sr0Percent = total === 0 ? 0 : (s0 / total) * 100;
    ...
    >
      ...
      <div className="flex-1 space-y-6">
        <div className="relative h-4 w-full bg-slate-100 rounded-full overflow-hidden flex">
          <div style={{ width: `${sr2Percent}%` }} className="bg-emerald-500 h-full" title="SR 2" />
          <div style={{ width: `${sr1Percent}%` }} className="bg-amber-400 h-full" title="SR 1" />
          <div style={{ width: `${sr0Percent}%` }} className="bg-slate-300 h-full" title="SR 0" />
          <div className="absolute left-[70%] top-0 bottom-0 w-0.5 bg-rose-500 z-10 opacity-70" />
        </div>

        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100">
            <p className="text-xl font-black text-emerald-600">{s2}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">SR 2</p>
          </div>
          <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
            <p className="text-xl font-black text-amber-600">{s1}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">SR 1</p>
          </div>
          <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
            <p className="text-xl font-black text-slate-400">{s0}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">SR 0</p>
          </div>
        </div>
      </div>
      <span className="text-[10px] font-bold text-orange-500 mt-4 text-center opacity-0 group-hover:opacity-100 transition-opacity">
        View Relevance Log
      </span>
    </motion.div>
  );
...
const AuthorityTierWidget = ({ stats, onClick }) => {
  ...
  const total = tA + tB + tC + tD;
  const aPercent = total === 0 ? 0 : (tA / total) * 100;
  const bPercent = total === 0 ? 0 : (tB / total) * 100;
  const cPercent = total === 0 ? 0 : (tC / total) * 100;
  const dPercent = total === 0 ? 0 : (tD / total) * 100;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 space-y-6">
        <div className="relative h-4 w-full bg-slate-100 rounded-full overflow-hidden flex">
          <div style={{ width: `${aPercent}%` }} className="bg-indigo-600 h-full" title="Tier A" />
          <div style={{ width: `${bPercent}%` }} className="bg-indigo-400 h-full" title="Tier B" />
          <div style={{ width: `${cPercent}%` }} className="bg-slate-300 h-full" title="Tier C" />
          <div style={{ width: `${dPercent}%` }} className="bg-rose-400 h-full" title="Tier D" />
        </div>

        <div className="grid grid-cols-4 gap-1 text-center">
          <div><p className="text-lg font-black text-indigo-600">{tA}</p><p className="text-[9px] font-bold text-slate-400">A</p></div>
          <div><p className="text-lg font-black text-indigo-400">{tB}</p><p className="text-[9px] font-bold text-slate-400">B</p></div>
          <div><p className="text-lg font-black text-slate-400">{tC}</p><p className="text-[9px] font-bold text-slate-400">C</p></div>
          <div><p className="text-lg font-black text-rose-400">{tD}</p><p className="text-[9px] font-bold text-slate-400">D</p></div>
        </div>
      </div>
      <span className="text-[10px] font-bold text-indigo-500 mt-4 text-center opacity-0 group-hover:opacity-100 transition-opacity">
        View Source Tiers
      </span>
    </motion.div>
  );
...
function AnalyticsPage() {
  ...
  const getFilteredRows = () => {
    ...
    if (modalType === 'COMPLIANCE') {
      if (activeFilter === "ALL") return data.compliance_details;
      return data.compliance_details.filter(item => item.tag === activeFilter);
    }

    // For Source Details (Codes, SR, Tier)
    const sourceRows = data.source_details;
    if (activeFilter === "ALL") return sourceRows;

    if (modalType === 'CODES') {
      return sourceRows.filter(item => item.codes[activeFilter] === true);
    }
    if (modalType === 'SR') {
      // Filter needs integer conversion for SR
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium text-sm">Real-time audit on {data?.meta?.total_facts || 0} facts from {data?.meta?.total_urls || 0} unique sources.</p>
            </div>
          </div>
          ...
          {loading || !data ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-1 space-y-6">
                ...
                <motion.div
                  whileHover={{ y: -5 }}
                  onClick={() => setModalType('CODES')}
                  className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-6 cursor-pointer hover:shadow-xl transition-all group"
                >
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                      <Activity size={16} className="text-indigo-500" /> Medical Codes
                    </h3>
                    <span className="text-[10px] font-bold text-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity">Details</span>
                  </div>

                  {/* Container for the lines */}
                  <div className="space-y-3">
                    <CodeBar label="SNOMED CT" count={data.codes.SNOMED} total={data.codes.SNOMED + data.codes.LOINC + data.codes["ICD-10"]} color="bg-blue-500" />
                    <CodeBar label="LOINC" count={data.codes.LOINC}  total={data.codes.SNOMED + data.codes.LOINC + data.codes["ICD-10"]} color="bg-amber-500" />
                    <CodeBar label="ICD-10" count={data.codes["ICD-10"]} total={data.codes.SNOMED + data.codes.LOINC + data.codes["ICD-10"]} color="bg-purple-500" />
                  </div>
                </motion.div>
              </div>
                  ...
                  <div className="space-y-4">
                    {data.functions.map((func, i) => (
                      <div key={i} className="flex items-center justify-between pb-4 border-b border-slate-50 last:border-0">
                        ...
                        <div className={`px-2 py-1 rounded-md text-[10px] font-black ${func.status === "Green" ? "bg-emerald-100 text-emerald-600" :
                      ...
                      <tbody className="text-sm font-bold text-slate-700 divide-y divide-slate-100">
                        ...
                        {modalType === 'COMPLIANCE' && filteredRows.map((log) => (
                          <tr key={log.id} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-4 px-6 max-w-xs truncate">
                              <a href={log.url} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-800 hover:underline">
                                {log.url}
                              </a>
                            </td>
                        ...
                        {(modalType === 'CODES' || modalType === 'SR' || modalType === 'TIER') && filteredRows.map((item) => (
                          <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-4 px-6 max-w-xs truncate">
                              <a href={item.url} target="_blank" rel="noreferrer" className="text-indigo-600 hover:text-indigo-800 hover:underline">
                                {item.url}
                              </a>
                            </td>
                            ...
                            {modalType === 'SR' && <>
                              <td className="py-4 px-6 text-center">
                                <span className={`px-3 py-1 rounded-full text-white text-xs ${item.sr_score === 2 ? "bg-emerald-500" : item.sr_score === 1 ? "bg-amber-400" : "bg-slate-300"
                                  }`}>
                                  {item.sr_score}
                                </span>
                              </td>
                            ...
                            {modalType === 'TIER' && <>
                              <td className="py-4 px-6 text-center">
                                <span className={`px-3 py-1 rounded-full text-white text-xs ${item.tier === "A" ? "bg-indigo-600" : item.tier === "B" ? "bg-indigo-400" : item.tier === "C" ? "bg-slate-400" : "bg-rose-400"
                                  }`}>
                                  {item.tier}
                                </span>
                              </td>
